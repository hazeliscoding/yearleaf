...
- [ ] **The scroll trap itself is still open.** The arrows are a way around it, not a fix for it: one space-drag upward of ~700px still goes September to December, and the navigator label reports exactly that pair with nothing between, because October and November are two columns to the left and never cross the screen on that path. That is the precise datum a professional calendar reader misread. Closing it needs a cue on the canvas as a pan crosses a month or a year boundary, not another toolbar control
- [ ] `ViewportStore.animate` is written in five places and read nowhere in the repo, so no fit, jump or step is animated despite `centerOn` documenting itself as animated. Months wrap on Mar→Apr, Jun→Jul, Sep→Oct and Dec→Jan, so one forward step in three shares no pixels at all with the frame before it — a hard cut where the signal to ease it is already plumbed
- [ ] Navigation leftovers: "Fit month" still fits the month rather than the working area around it, the year view has no day numbers or today marker, there is no horizontal wheel panning, and the step shortcuts are advertised only in the arrows' tooltips — not in the command palette, which already renders shortcut chips
